...
# ================== حالة الجلسة والمستويات ==================
if "completed" not in st.session_state:
    st.session_state.completed = set()  # مفاتيح الحالات المحلولة صحيحاً

CASES = {
    "case1": "قصر النظر الشديد مع الاستجماتيزم (Vertex Distance)",
    "case2": "القرنية المخروطية (Keratoconus)",
    "case3": "ارتفاع ضغط العين مقابل الجلوكوما (Ocular Hypertension vs NTG)",
    "case4": "كسل العين التبايني (Anisometropic Amblyopia)",
}

LEVELS = [  # (الحد الأدنى للحالات، الأيقونة، الاسم، الشرط)
    (0, "🌱", "مبتدئ", "0 حالة"),
    (1, "📗", "متدرب", "1+ حالة صحيحة"),
    (3, "🚀", "متقدم", "3 حالات صحيحة"),
    (4, "🏆", "خبير إكلينيكي", "الحالات الأربع كاملة + 🎈 احتفال"),
]
...
section = st.sidebar.radio("اختر القسم:", [
    ...
    "6. المستوى والشهادة (Progress & Certificate)"
])
progress_slot = st.sidebar.empty()  # يُملأ في نهاية السكربت بعد تحديث التقدم
...
# =========================================================
# القسم السادس: المستوى والشهادة
# =========================================================
elif "6." in section:
    st.header("🏆 6. المستوى والشهادة (Progress & Certificate)")

    done = st.session_state.completed
    n = len(done)
    cur = get_level(n)

    st.subheader("مستواك الحالي")
    st.markdown(f"### {cur[1]} {cur[2]}  —  {n}/4 حالات صحيحة")
    st.progress(n / 4)

    rows = ["| المستوى | الشرط |", "|---------|-------|"]
    for _, icon, name, cond in LEVELS:
        mark = "  ◀ مستواك الحالي" if name == cur[2] else ""
        rows.append(f"| {icon} {name}{mark} | {cond} |")
    st.markdown("\n".join(rows))

    st.subheader("الحالات المكتملة")
    for key, title in CASES.items():
        st.markdown(("✅ " if key in done else "⬜ ") + title)

    st.subheader("🎓 شهادة الإتمام")
    if n < 4:
        st.info("🔒 تُفتح بوابة إصدار الشهادة عند إتمام الحالات الأربع (4/4) بإجابات صحيحة.")
    else:
        st.success("🎉 أتممتَ الحالات الأربع! يمكنك الآن إصدار شهادتك الرسمية.")
        font_name, has_shaping = cert.font_status()
        if not has_shaping:
            st.warning("مكتبتا arabic-reshaper و python-bidi غير مثبتتين، وبدونهما لن يظهر العربي في الشهادة سليماً. أضِفهما إلى requirements.txt.")
        if font_name is None:
            st.warning("لم يُعثر على أي خط عربي؛ ضع Amiri-Regular.ttf و Amiri-Bold.ttf داخل assets/fonts/.")
        elif font_name != "Amiri":
            st.info(f"لم يُعثر على خط Amiri داخل assets/fonts/ — استُخدم خط النظام ({font_name}). ضع Amiri لأفضل جودة عربية.")

        trainee = st.text_input("اسم المتدرب كما سيظهر في الشهادة:", max_chars=60, key="cert_name")
        if trainee.strip():
            issued = st.session_state.get("completed_on", datetime.date.today())
            cred_id = cert.credential_id(trainee, done, issued)
            try:
                pdf_bytes = cert.build_certificate(trainee, [CASES[k] for k in CASES if k in done], issued, cred_id)
            except Exception as exc:
                st.error(f"تعذّر إنشاء الشهادة: {exc}")
            else:
                st.download_button("📄 تحميل الشهادة (PDF)", data=pdf_bytes,
                                   file_name=f"OptoAcademy-Certificate-{cred_id}.pdf",
                                   mime="application/pdf")
                st.caption(f"Credential ID: {cred_id}")

                st.subheader("💼 نقل الشهادة إلى LinkedIn")
                st.link_button("💼 Add to Profile", cert.linkedin_url(issued, cred_id))
                st.caption("إن لم تُملأ الحقول تلقائياً، انسخ القيم الجاهزة:")
                st.code(cert.LINKEDIN_CERT_NAME, language=None)
                st.code(cert.LINKEDIN_ORG, language=None)
                st.code(f"{issued.month:02d}/{issued.year}", language=None)
                st.code(cred_id, language=None)

# ================== تحديث التقدم (الشريط الجانبي + الاحتفال) ==================
_n = len(st.session_state.completed)
if _n == 4 and "completed_on" not in st.session_state:
    st.session_state.completed_on = datetime.date.today()
    st.balloons()  # 🎈 احتفال بإتمام 4/4

_icon, _level = get_level(_n)[1], get_level(_n)[2]
with progress_slot.container():
    st.markdown("---")
    st.markdown(f"**المستوى:** {_icon} {_level}")
    st.progress(_n / 4, text=f"{_n}/4 حالات صحيحة")
    if _n == 4:
        st.success("🎓 الشهادة جاهزة — افتح القسم 6")
